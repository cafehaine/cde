...
impl ToString for KanshiProfile {
    fn to_string(&self) -> String {
        ...
        let config = Vec::from_iter(str_directives).join("\n");
        match &(self.name) {
...
impl KanshiProfile {
    pub fn from_layout(screen_layout: &Vec<Output>) -> KanshiProfile {
        let directives = Vec::from_iter(
            screen_layout
                .iter()
...
fn _build_directive(pair: Pair<Rule>) -> KanshiDirective {
    let words = Vec::from_iter(pair.into_inner().map(|p| p.as_str().to_string()));
    ...
    pub fn load() -> Self {
        ...
        match _load_kanshi_config(&xdg_dirs) {
            ...
            None => match xdg_dirs.place_config_file("config") {
                Ok(path_buf) => Self {
                    config_path: Box::new(path_buf.to_owned()),
                    profiles: Vec::new(),
                },
                Err(err) => panic!(
